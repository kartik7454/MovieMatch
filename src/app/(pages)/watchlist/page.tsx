  ...
  useEffect(() => {
    const fetchWatchlistMovies = async () => {
      ...
      try {
        ...
        if (data.success) {
          setWatchlistMovies(data.watchlist_movies || []);
        } else {
          setError(data.error || "Failed to fetch watchlist movies");
        }
      } catch (err: any) {
          ...
          >
            {loading ? (
              ...
              <div className="col-span-full text-center py-10">
                ...
                <p className="mt-2 text-white/70">
                  Add some movies to build your watchlist!
                </p>
              </div>
            ) : (
              watchlistMovies.map((item: any) => (
                <div key={item.id} className="flex flex-col group items-stretch">
                  <div className="relative">
                    ...
                    />
                    {/* Remove Button Overlay (appears on hover, top right corner, with animation) */}
                    <button
                      ...
                      onClick={() => {
                        setWatchlistMovies(prev =>
                          prev.filter((movie: any) => movie.id !== item.id)
                        );
